import { observer } from 'mobx-react-lite'
import React, { useContext, useEffect } from 'react'
import businessStore from '../../../stores/businessStore';
import {Button, Card, Col, Divider, Row, Space} from 'antd';
import { RouteComponentProps } from 'react-router';
import Avatar from 'antd/lib/avatar/avatar';
import { ArrowLeftOutlined, MailOutlined, PhoneOutlined, ShopTwoTone, UserOutlined } from '@ant-design/icons';
import { Link } from 'react-router-dom';

interface DetailParams{
    id: string
}

const BusinessDetails: React.FC<RouteComponentProps<DetailParams>> = ({ match}) => {
    const {Meta} = Card;
    const BusinessStore = useContext(businessStore); 
    const {business, loadBusiness, loadingInitial} = BusinessStore;
    useEffect(() => {
        loadBusiness(match.params.id);
    }, [loadBusiness, match.params.id])
    return (
        <div>
            <Row>
                <Col span = {3}/>
                <Col>
                   <Link to = {'/Businesses'}>
                        <Button style = {{marginBottom : '1em'}}>
                            <ArrowLeftOutlined style = {{marginRight: '1em'}}/>
                            Return to list of businesses
                        </Button>
                   </Link>

                </Col>
            </Row>
            <Row>
                <Col span = {3}>
                </Col>
                <Col span = {9} >
                    <Row>
                        <img src = "data:image/png;base64,"
                            style = {{width: "100%"}}
                        />
                    </Row>
                    <Row style = {{marginTop: "1em"}}>
                        <Card style = {{width: '100%'}}>
                            <h2>About {business?.name}</h2>
                            <Divider/>
                            <Card style = {{background: "#f5f5f5", marginBottom: "3em"}}>
                                <h3>Offered Services</h3>
                                <p>{business?.description2}</p>
                            </Card>

                            <p>{business?.description3}</p>
                        </Card>
                                
                    </Row>                    

                </Col>
                <Col span = {8} style = {{marginLeft: "1em"}}>
                    
                    <Card>
                        <Meta 
                           title = {business?.name}
                        />
                        <Divider/>
                        {business?.photos.length !== 0 && (
                            <img width = {'100%'}
                            height = {'200px'}
                            alt = "logo"
                            src ={business?.photos[0]?.url || "assets/placeholder.png"} 
                            />
                        )}

                        <Divider/>
                        <p>{business?.description}</p>
                        <Divider/>
                        <div style = {{textAlign: "center"}}> 
                            <Card>
                                <Button style = {{background: "#4FCE5D", color: "white", width: "200px", height: "50px" }}>
                                    <PhoneOutlined style = {{marginRight: "1em"}}/>
                                    {business?.phones[0]?.number}
                                </Button>
                                <div>
                                    {business?.phones[0]?.description !== null && (
                                        <div>
                                            <UserOutlined style = {{marginRight: "1em"}}/>
                                            {business?.phones[0]?.description}  
                                        </div>
  
                                    )}
  
                                </div>
                          
                            </Card>
                            <br></br>
                            {business?.phones.length! > 1 && (
                                <Card>
                                    <Button style = {{background: "#4FCE5D", color: "white", width: "200px", height: "50px" }}>
                                        <PhoneOutlined style = {{marginRight: "1em"}}/>
                                        {business?.phones[1]?.number}
                                    </Button>
                                    {business?.phones[1]?.description !== null && (
                                        <div>
                                            <UserOutlined style = {{marginRight: "1em"}}/>
                                            {business?.phones[1]?.description}      
                                        </div>
                                    )}

                                </Card>
                            )}                           

                            

                        </div>
                        <div style = {{textAlign: "center"}}>
                            {business?.website !== null && (
                            <Button size = "large"
                            style = {{margin: "3px", background: "orange", color: "white", border: "none"}}   
                            onClick = {() => window.open(`//www.${business?.website}`)}
                            >
                                View Website 
                            </Button>
                        )}
                        </div>
                        <div style = {{marginTop: "1em"}}>
                            {business?.emails.length !== 0 && (
                                <div>
                                    <MailOutlined style = {{marginRight: '5px'}}/>
                                    {business?.emails[0]?.emailAddress}
                                </div>

                            )}
                            {business?.addresses.length !== 0 && (
                                <div style = {{ marginTop: "5px"}}>
                                    <ShopTwoTone style ={{marginRight: "5px"}}/> 
                                    {business?.addresses[0]?.street} {business?.addresses[0]?.suburb} {business?.addresses[0]?.state} {business?.addresses[0]?.postcode}  
                                
                                </div>
                            )}
                        </div>
                        <Divider/>
                        *Trading hours go here

                    </Card>


                </Col>
            </Row>

        </div>
    )
}


export default observer(BusinessDetails);